...
function revealField(field, element) {
  ...
  switch(true) {
    case field.value == 0: {
      ...
      corners.forEach(z => {
        let id = `#${z[0]}_${z[1]}`
        cssReveal($(id)[0], fields[z[1]][z[0]].value)
      })
...
function increaseScroreBy(val) {
  let current = Number($('#score')[0].innerText)
...
function changeBombCount(n) {
  let current = Number($('#bombs')[0].innerText)
  let now = current + n
  $('#bombs')[0].innerText = now
}
